...
def _validate_expiry_cycle(
    ...
    if configured_user_id != config.user_id:
        raise ValueError("Initial audit user does not match configured user")
    ...
    if dependencies.get_meta(connection, "initial_audit_cycle_started_at") is None:
        raise ValueError(
            "Response cycle has not started; run initial-audit-start first"
        )
    ...
    if (
        ...
        and (
            applied_as_of != window.resolved_at
            or applied_hours != window.hours_text
        )
    ):
        raise ValueError(
...
def _expiry_event_time(
    ...
    if created_at_utc > window.current_utc:
        counts["future_timestamp"] += 1
        return None
    if created_at_utc >= window.cutoff:
        counts["within_window"] += 1
        return None
    return created_at_utc


def _expire_one_event(
    connection: sqlite3.Connection,
    event: sqlite3.Row,
    *,
    ...
    try:
        ...
        evidence = json.dumps(
            [
                ...
                f"response_window_hours:{window.hours_text}",
                f"created_at:{isoformat(created_at_utc)}",
                f"cutoff:{window.cutoff_text}",
            ],
            ensure_ascii=False,
            sort_keys=True,
        )
...
def _expire_candidates(
    ...
    cursor = _expiry_candidate_cursor(config, connection)
    while True:
        ...
        for event in rows:
            counts["examined"] += 1
            created_at_utc = _expiry_event_time(
                event,
                window,
                counts,
                blocked_candidates,
            ...
            if len(expired_sample) < 20:
                expired_sample.append(str(event["event_id"]))
    return counts, blocked_candidates, expired_sample


def _commit_expiry_cycle(
    connection: sqlite3.Connection,
    window: ExpiryWindow,
    dependencies: Dependencies,
) -> None:
    for key, value in (
        ("initial_audit_response_window_hours", window.hours_text),
        ("initial_audit_last_expire_cutoff", window.cutoff_text),
        ("initial_audit_last_expire_at", window.resolved_at),
        ("initial_audit_cycle_expiry_as_of", window.resolved_at),
        ("initial_audit_cycle_expiry_hours", window.hours_text),
    ):
        dependencies.set_meta(connection, key, value)
    connection.commit()
...
def expire_initial_audit_events(
    ...
    *,
    ...
    window = _expiry_window(response_window_hours, now=now)
    transaction_started = False
    try:
        connection.execute("BEGIN IMMEDIATE" if not dry_run else "BEGIN")
        transaction_started = True
        _validate_expiry_cycle(config, connection, window, dependencies)
        counts, blocked_candidates, expired_sample = _expire_candidates(
            config,
            connection,
            window,
        )
        if dry_run:
            ...
            _commit_expiry_cycle(connection, window, dependencies)
        transaction_started = False
    except Exception:
    ...
    return {
        ...
        "response_window_hours": window.hours,
        "cutoff": window.cutoff_text,
        **counts,
...
def _mandatory_requeue_window(
    ...
    *,
    ...
    return current, current - timedelta(hours=response_window_hours)


def _mandatory_requeue_candidates(
    config: Any,
    connection: sqlite3.Connection,
    *,
    current: datetime,
    ...
    for row in rows:
        ...
        if created_at_utc < cutoff or created_at_utc > current:
            continue
        ignored_mention_reply = (
            row["disposition"] is None
            and row["delivery_state"] == "ignored"
            and bool(row["is_reply"])
        )
        if not ignored_mention_reply and not dependencies.is_eligible_reply(
            connection,
            config,
            ...
            continue
        if watcher_resolution._matching_alex_reply_turns(
            connection,
            str(row["event_id"]),
        ):
            continue
...
def requeue_unanswered_skips(
    ...
    *,
    ...
    current, cutoff = _mandatory_requeue_window(
        config,
        response_window_hours=response_window_hours,
        now=now,
    )
    candidates = _mandatory_requeue_candidates(
        config,
        connection,
        current=current,
        cutoff=cutoff,
    ...
    return {
        ...
        "candidate_event_ids": [str(row["event_id"]) for row in candidates],
...
def _resolved_history_missing_count(
    connection: sqlite3.Connection,
    user_id: str,
) -> int:
    return int(
...
def _published_history_missing_count(
    connection: sqlite3.Connection,
    user_id: str,
) -> int:
    return int(
...
def _validate_audit_completion(
    connection: sqlite3.Connection,
    user_id: str,
) -> None:
    unresolved = _unresolved_direct_count(connection, user_id)
    if unresolved:
        raise ValueError(
            "Initial audit cannot complete with "
            f"{unresolved} unresolved direct events"
        )
    history_missing = _resolved_history_missing_count(connection, user_id)
    if history_missing:
        raise ValueError(
            "Initial audit cannot complete with "
            f"{history_missing} resolved events missing exact history turns"
        )
    published_missing = _published_history_missing_count(connection, user_id)
    if published_missing:
        raise ValueError(
            "Initial audit cannot complete with "
            f"{published_missing} published replies missing matching exact "
            "Alex history turns"
        )
...
def complete_initial_audit(
    ...
    *,
    ...
    _validate_audit_completion(connection, config.user_id)
