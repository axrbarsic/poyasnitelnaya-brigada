...
Новая отдельная система уборки не создана. Канонический `session_janitor` уже
сопоставляет helper с завершённой automation по времени запуска, защищает
активные owner thread ID и завершает только вычисленный список. Lightpanda MCP
добавлен в этот существующий allowlist. System doctor теперь требует свежий
janitor state и нулевой `helper_survivors`, поэтому повторная утечка становится
видимым FAIL, а не скрытой деградацией ресурсов.

Для уже архивированных задач предусмотрен отдельный ручной recovery-флаг. Он
читает архив только для точного сопоставления времени запуска helper, не
возвращает задачи в интерфейс и не меняет их. Обычный минутный запуск архив не
сканирует. Если helper cleanup вернул ошибку или survivor, janitor теперь не
архивирует исходную задачу, чтобы следующий запуск сохранил возможность
безопасного повтора.
