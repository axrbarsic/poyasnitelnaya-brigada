...
1. Relay направлял follow-up в длинную пользовательскую задачу, где служебная
   работа могла ждать завершения другого turn.
2. Маршрут содержал зашитые `threadId` и `hostId`. После обновления приложения
   старый внутренний host перестал быть устойчивой частью адресации.
...
Сейчас основная пользовательская задача осталась одна. Для автоматики выделен
один ясно названный worker `X: служебный worker`. Остальные ошибочные и
дублирующие задачи архивированы.
...
flowchart LR
    ...
    D --> R["Luna relay reservation"]
    R --> O["Sol Max service worker"]
...
| --- | --- |
| Старый адрес worker | `relay-reserve-handoff` читает owner из versioned contract и сверяет его с config |
| Нестабильный внутренний host | Relay больше не передаёт `hostId` |
...
- Luna получает только короткий relay turn, когда готова реальная очередь. Если
  Alex держит Desktop открытым, heartbeat всё равно выполняет небольшой gate.
- Sol Max запускается только для реального события, которое требует решения.
