...
как локальный процесс, запускаемый командой. Официальный
[Codex app-server](https://learn.chatgpt.com/docs/app-server.md) связывает MCP с
загруженной задачей и выгружает задачу только после исчезновения подписчиков и
периода отсутствия активности.

Проект больше не пытается выводить принадлежность `lightpanda_mcp.py` по
совпадению времени запуска задачи и процесса. При параллельных задачах такая
эвристика неоднозначна и может завершить чужой helper. Ротация архивирует точную
старую owner-задачу официальным Codex API, doctor проверяет единственность
нового owner, а resource guard откладывает Browser при временном давлении
памяти. Legacy helper очищаются перезапуском Codex Desktop на безопасной
границе.
