...
Экспериментальный v2 хранится и устанавливается отдельно:

- источник: `skill-backup/poyasnitelnaya-brigada-v2`;
- установленная копия: `~/.codex/skills/poyasnitelnaya-brigada-v2`;
- v1 не удаляется и не мигрируется автоматически;
- v2 включается только по прямому запросу Alex или для уже помеченной v2
  цепочки.
