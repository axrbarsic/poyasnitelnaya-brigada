...
4. Локальный dispatcher выдает всей очереди одну глобальную 30-минутную аренду
   владельца.
5. Запланированная задача Luna Low раз в пять минут выполняет только
   read-only `gate`. Пустая, занятая или отложенная очередь завершается без
   Sol и Browser.
6. Только готовая очередь пробуждает одну закрепленную сессию Browser owner на
   Sol High. Она атомарно выполняет claim и становится единственным владельцем.
7. Sol High открывает живую ветку X, анализирует текст и изображения, проверяет
   первичные источники, историю диалога и отсутствие дубля.
8. Короткий ответ пишет Sol High. Длинный follow-up может продолжаться только в
   точной исторической сессии кастомного GPT.
9. После публикации сохраняются точный URL, текст, источники и durable
...
- Пустой проход использует Luna Low. Sol High получает токены только при
  готовой непустой очереди.
...
После этого установите три LaunchAgent, закрепленную сессию Sol High и
запланированный Luna Low dispatcher по
...
звена. Dry-run и apply получают один и тот же `as-of`, после чего обычный
Luna dispatcher должен сам разбудить Sol owner. Известный event ID нельзя
